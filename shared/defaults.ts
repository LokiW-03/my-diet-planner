...
export const defaultCategoryId = (name: string) =>
  `cat:${name}` as unknown as CategoryId;
...
export const defaultMealId = (name: string) =>
  `meal:${name}` as unknown as MealId;
...
const catId = (name: string) =>
  catIdByName.get(name) ?? defaultCategoryId(CATEGORIES[0]);
...
export function createEmptyMealsState(): Record<MealKey, MealEntry[]> {
  return Object.fromEntries(MEALS.map((k) => [k, [] as MealEntry[]])) as Record<
    MealKey,
    MealEntry[]
  >;
}

export const defaultTargetId = (name: string) =>
  `target:${name}` as unknown as TargetId;
...
export const defaultMeals: Record<MealId, MealDefinition> = Object.fromEntries(
  DEFAULT_MEALS.map((m) => [m.id, m]),
) as Record<MealId, MealDefinition>;

export const defaultFoods: Record<FoodId, FoodItem> = Object.fromEntries(
  DEFAULT_FOODS.map((f) => [f.id, f]),
) as Record<FoodId, FoodItem>;

export const defaultCategories: Record<CategoryId, FoodCategory> =
  Object.fromEntries(DEFAULT_CATEGORIES.map((c) => [c.id, c])) as Record<
    CategoryId,
    FoodCategory
  >;
